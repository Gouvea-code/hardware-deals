...
import {useSessionStore} from '../store/sessionStore';import {colors} from '../theme';import {trackEventSafely} from '../services/analyticsService';
export function SessionBootstrap({children}:PropsWithChildren){const hydrated=useSessionStore(s=>s.hydrated);const hydrate=useSessionStore(s=>s.hydrate);
 useEffect(()=>{trackEventSafely('APP_OPEN');hydrate().catch(()=>undefined);},[hydrate]);if(!hydrated)return <View style={styles.root}><ActivityIndicator color={colors.primary} size="large"/></View>;return children;}
